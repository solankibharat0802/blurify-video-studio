...
//   // Reset all states when modal opens with a different file
//   useEffect(() => {
//     if (file && isOpen) {
...
import { Slider } from "@/components/ui/slider";
import { Label } from "@/components/ui/label";
import { Play, Pause, Download, Plus, Trash2, Square } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
...
export const VideoEditModal = ({ isOpen, onClose, file, onSaveEdit }: VideoEditModalProps) => {
  ...
  const handleMouseUp = () => {
    ...
    if (width > 10 && height > 10) {
      ...
      toast({ title: "Blur mask added", description: `Mask at ${formatTime(currentTime)}` });
    }
    setIsDrawing(false);
  };
  ...
  const updateMask = (id: string, updates: Partial<BlurMask>) => {
    setBlurMasks((prev) => prev.map((mask) => (mask.id === id ? { ...mask, ...updates } : mask)));
  };
  ...
  // ✅ Save masks and send to backend
  const handleSave = async () => {
    if (blurMasks.length === 0) {
      toast({ title: "No masks", description: "Add at least one blur mask", variant: "destructive" });
      return;
    }
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/process-video", {
        ...
        body: JSON.stringify({
          videoUrl: file?.supabaseUrl || null,
          masks: blurMasks,
        }),
      });
      ...
      toast({ title: "Processing started", description: "Blur will be applied" });
      onSaveEdit(blurMasks);
      onClose();
    } catch (err: any) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Click and drag on the video to create blur masks, then adjust settings.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 flex gap-6">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="mt-4 flex items-center gap-4">
              ...
              <Slider value={[currentTime]} onValueChange={handleSeek} max={duration} step={0.1} className="flex-1" />
          ...
          <div className="w-80 space-y-6">
            <Card className="p-4">
              <h3 className="font-semibold mb-4">Blur Masks ({blurMasks.length})</h3>
              {blurMasks.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">No blur masks yet.</p>
              ) : (
                <div className="space-y-3 max-h-60 overflow-y-auto">
                  {blurMasks.map((mask, i) => (
                    <Card
                      key={mask.id}
                      className={`p-3 cursor-pointer ${selectedMask === mask.id ? "border-primary bg-primary/5" : ""}`}
                      onClick={() => setSelectedMask(mask.id)}
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-sm">Mask {i + 1}</span>
                      ...
                      <div className="space-y-2 mt-2">
                        ...
                        <Slider
                          value={[mask.intensity]}
                          onValueChange={(v) => updateMask(mask.id, { intensity: v[0] })}
                          max={50}
                          min={1}
                          step={1}
                        />
