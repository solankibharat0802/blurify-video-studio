...
const getBackendUrl = () => "http://127.0.0.1:5000";
...
export const UploadSection = () => {
  ...
  const uploadToBackend = async (file: File, fileId: string): Promise<string> => {
    ...
    formData.append('video', file); formData.append('videoId', fileId);
    const response = await fetch(`${getBackendUrl()}/upload-video`, { method: 'POST', body: formData });
    if (!response.ok) throw new Error(`Upload failed with status: ${response.status}`);
    const result = await response.json();
    if (!result.success || !result.videoId) throw new Error('Backend did not confirm upload success.');
  ...
  const handleFiles = useCallback(async (droppedFiles: File[]) => {
    ...
    const newFilesPromises = videoFiles.map(async (file) => {
      ...
      try {
        const [preview, videoId] = await Promise.all([ createVideoPreview(file), uploadToBackend(file, fileId) ]);
  ...
  const handleSaveEdit = async (masks: BlurMask[]) => {
    if (!editingFile || !editingFile.videoId) return;
